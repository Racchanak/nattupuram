...
else
{
    ...
    if(isset($_POST['submit']))
    {
        ...
        if($edit_id != 0){
           ...
           $ins_pack = ' UPDATE `products` SET `product_name`="' . $product_name . '",`main_img`="'. $main_img_path . '",`image1`="' . $image1_path . '",`image2`="' . $image2_path . '",`quantity`="' . $quantity . '",`sub_quantity`="' . $sub_quantity . '",`price`="' . $price . '",`description`="' . $description . '",`offers`="' . $offer . '",`review_count`="' . $review_count . '",`pro_type`="' . $pro_type . '",`gst_val`="' . $gst_val . '" WHERE `product_id`= "'.$edit_id.'"';
            mysqli_query($GLOBALS['link'], $ins_pack);
            $success = "Inserted Successfully";

            $delete_characteristics = "DELETE FROM product_characteristics WHERE product_id = $edit_id";
            mysqli_query($GLOBALS['link'], $delete_characteristics);

            $delete_benefits = "DELETE FROM product_benefits WHERE product_id = $edit_id";
            mysqli_query($GLOBALS['link'], $delete_benefits);

            $delete_methods = "DELETE FROM product_methods WHERE product_id = $edit_id";
            mysqli_query($GLOBALS['link'], $delete_methods);

            $delete_facts = "DELETE FROM product_facts WHERE product_id = $edit_id";
            mysqli_query($GLOBALS['link'], $delete_facts);


            foreach ($product_characteristics as $product_characteristics_value) {
                $sql = "INSERT INTO product_characteristics (product_id, description) VALUES ('$edit_id', '$product_characteristics_value')";
                mysqli_query($GLOBALS['link'], $sql);
            }

            foreach ($product_benefits as $product_benefits_value) {
                $sql = "INSERT INTO product_benefits (product_id, description) VALUES ('$edit_id', '$product_benefits_value')";
                mysqli_query($GLOBALS['link'], $sql);
            }

            foreach ($product_methods as $product_methods_value) {
                $sql = "INSERT INTO product_methods (product_id, description) VALUES ('$edit_id', '$product_methods_value')";
                mysqli_query($GLOBALS['link'], $sql);
            }
              ...
              if($status != '') {
                if($i < sizeof($fact_result))
                {
                  if($j < sizeof($product_facts))
                  {
                      ...
                      $sql = "INSERT INTO product_facts (product_id, facts_description, fact_result) VALUES ('$edit_id', '$p_facts', '$f_result')";
                      mysqli_query($GLOBALS['link'], $sql);
                      $j++;
                  }
                    $i++;
          ...
          <form role="form" id="social" name="social" class="form-inline" action="update-product.php" enctype="multipart/form-data" method="post" onSubmit="return product_validate()">
             ...
             <div class="row">
                <div class="col-lg-6 col-md-6 pad-rt-0">
                    <div class="form-group error-msg">
                        <h5>product Name</h5>
                        <input type="text" placeholder="Product Name" id="product_name" name="product_name" class="form-control checksco" value="<?php if(isset($product_name)){ echo $product_name; } else { echo ""; } ?>">
                ...
                <div class="col-lg-6 col-md-6 pad-rt-0">
                    <div class="form-group error-msg">
                        <h5>GST Value</h5>
                        <input type="text" placeholder="GST Value" id="gst_val" name="gst_val" class="form-control checksco" value="<?php if(isset($gst_val)){ echo $gst_val; } else { echo ""; } ?>">
            ...
            <div class="row">
                <div class="col-lg-6 col-md-6 pad-rt-0">
                    <div class="form-group error-msg">
                        <h5>Review Count</h5>
                        <input type="text" placeholder="Review Count" id="review_count" name="review_count" class="form-control checksco" value="<?php if(isset($review_count)){ echo $review_count; } else { echo ""; } ?>" onkeypress="return isNumberKey(event);">
                ...
                <div class="col-lg-6 col-md-6 pad-rt-0">
                    <div class="form-group error-msg">
                        ...
                        <select class="form-control checksco" id="product_type" name="product_type" onchange="show_product_details()">
                            ...
                            <option value="oil" <?php if($pro_type == "oil"){ echo "selected"; } ?>>Oil</option>
                            <option value="ghee" <?php if($pro_type == "ghee"){ echo "selected"; } ?>>Ghee</option>
                        </select>
                ...
                <div class="col-lg-6 pad-lft-0 key-highlight">
                    <div class="form-group error-msg">
                        <h5>Add Product Characteristics</h5>
                            <?php                           
                            if(isset($product_characteristics) && !empty($product_characteristics)){
                                foreach ($product_characteristics as $value) {
                                    echo '<input  name="product_characteristics[]" type="text" class="form-control custom-ht" value = "'.$value['description'].'"/>';
                                }
                            }
                            ?>
                ...
                <div class="col-lg-6 pad-lft-0 key-highlight">
                    <div class="form-group error-msg">
                        <h5>Add Product Benefits</h5>
                             <?php                           
                            if(isset($product_benefits) && !empty($product_benefits)){
                                foreach ($product_benefits as $value) {
                                    echo '<input  name="product_benefits[]" type="text" class="form-control custom-ht" value = "'.$value['description'].'"/>';
                                }
                            }
                            ?>
                ...
                <div class="col-lg-6 pad-lft-0 key-highlight">
                    <div class="form-group error-msg">
                        <h5>Add Product Methods</h5>
                            <?php                           
                            if(isset($product_methods) && !empty($product_methods)){
                                foreach ($product_methods as $value) {
                                    echo '<input  name="product_methods[]" type="text" class="form-control custom-ht" value = "'.$value['description'].'"/>';
                                }
                            }
                            ?>
            ...
            <div class="row">
                <div class="col-lg-6 pad-lft-0 key-highlight">
                    <div class="form-group error-msg">
                        <h5>Offers</h5>
                        <?php
                            $record = explode(",",$offers);
                            foreach ($record as $value) {
                                echo '<div class="form-group"><input  placeholder="Add Offer" name="offer[]" type="text" class="form-control custom-ht" value="'.$value.'"/></div>';
                            }
                        ?>
            ...
            <div class="buttons pull-right">
                ...
                <a href="update-product.php" class="save-publish">Cancel</a>
            </div>
          </form>
        </div>
      </div>
...
?>
    ...
    function product_validate() {
        ...
        var hidden_main_img = $('#main_img').val();
